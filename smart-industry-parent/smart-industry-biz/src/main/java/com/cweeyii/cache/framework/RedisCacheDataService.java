...
@Service
public abstract class RedisCacheDataService<K,V> extends AbstractDataReader<K,V>{

    private static final Logger LOGGER = LoggerFactory.getLogger(RedisCacheDataService.class);
...
//                V v = resultMap.get(entry.getKey());
    ...
    protected void assertCacheType(Class<?> viewClass) {
        ...
        CacheDataType dataType = annotation.value();
        if (dataType == null) {
            throw new AssertionError("Medis view 's annotation CacheEntity 's dataType is null");
        }
        if (this instanceof MapCacheDataService) {
            if (CacheDataType.Object.equals(dataType)) {
                throw new AssertionError("MapCacheDataService 's operation 's param is CacheDataType.Object");
            }
        } else if (this instanceof ObjectCacheDataService) {
            if (CacheDataType.Map.equals(dataType)) {
                throw new AssertionError("ObjectCacheDataService 's operation 's param is CacheDataType.Map");
            }
        }
    }
