...
import com.cweeyii.baseinfo.core.constant.PoiConstant;
...
public class PoiUtil {
    ...
    public static float calDistanceSim(StagePoi base, StagePoi target) {
        if (base.getLatitude() == null || base.getLatitude() < 10 || target.getLatitude() == null || target.getLatitude() < 10) {
            return 0;
        }
        float distance = PoiUtil.getDistance(base.getLatitude(), base.getLongitude(),
                target.getLatitude(), target.getLongitude()).floatValue();
        if (distance > 500) {
            return 0;
        }
        if(distance<50){
    ...
    public static StagePoi copyPoi(StagePoi poi) {
        StagePoi sp = clone(poi);
        if(sp == null) {
            throw new IllegalStateException("clone poi fail");
        }
        return sp;
    }
...
//        }
    ...
    public static Set<String> getModifiedSet(StagePoi src, StagePoi dst) {
        Set<String> modifiedSet = Sets.newHashSet();
        if (!dst.getAddress().equals(src.getAddress())) {
            modifiedSet.add(PoiConstant.Fields.ADDRESS);
        }
        if (!dst.getBareaId().equals(src.getBareaId())) {
            modifiedSet.add(PoiConstant.Fields.BAREAID);
        }
        if (!dst.getLocationId().equals(src.getLocationId())) {
            modifiedSet.add(PoiConstant.Fields.LOCATIONID);
        }
        if (!dst.getLatitude().equals(src.getLatitude())
                || !dst.getLongitude().equals(src.getLongitude())) {
            modifiedSet.add(PoiConstant.Fields.LATLNG);
        }
        if (!dst.getPhone().equals(src.getPhone())) {
            modifiedSet.add(PoiConstant.Fields.PHONE);
        }
        if (!dst.getPointName().equals(src.getPointName())) {
            modifiedSet.add(PoiConstant.Fields.POINTNAME);
        }
        if (!dst.getTypeId().equals(src.getTypeId())) {
            modifiedSet.add(PoiConstant.Fields.TYPEID);
        }
        if (dst.getCloseStatus() != src.getCloseStatus()){
            modifiedSet.add(PoiConstant.Fields.CLOSESTATUS);
        }
        return modifiedSet;
    }

    public static boolean onlyModifyLatLng(StagePoi src, StagePoi dst){
        Set<String> modifiedSet = getModifiedSet(src, dst);
        return modifiedSet.size() == 1&& modifiedSet.contains(PoiConstant.Fields.LATLNG);
    }
